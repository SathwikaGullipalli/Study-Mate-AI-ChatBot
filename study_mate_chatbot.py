...
def extract_text_from_pdf(pdf_file):
    ...
    try:
        ...
        return f"✅ PDF uploaded successfully! Extracted {len(text)} characters from {len(pdf_reader.pages)} pages."
...
def chatbot_response(message, history):
    ...
    if not pdf_text_content:
        response = "⚠️ Please upload a PDF first to ask questions about it!"
    else:
...
def generate_voice_explanation(text_to_speak):
    ...
    try:
        if not text_to_speak or text_to_speak.strip() == "":
            return "⚠️ Please enter text to convert to speech!"
...
with gr.Blocks(title="Study Mate Chatbot", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        with gr.Column():
            pdf_input = gr.File(label="📄 Upload Your Study Material (PDF)", file_types=[".pdf"])
    ...
    with gr.Tabs():
        ...
        # Tab 1: Chatbot Q&A
        with gr.Tab("💬 Chat & Ask Questions"):
...
print("\n" + "="*50)
print("🚀 Launching Study Mate Chatbot...")
print("="*50 + "\n")
